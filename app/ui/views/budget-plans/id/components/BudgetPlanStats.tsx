...
import { SquareIcon, SearchIcon, FilterIcon, SortByIcon, OrderByIcon, GroupByIcon, TrashIcon, ChevronRightIcon, ChevronDownIcon, ChevronUpIcon } from "@/app/ui/Icons";
...
function BugetPlanStatsOptionMenu(
    ...
    return (
        <div className="absolute flex flex-col right-0 min-w-[12rem] z-10 bg-dark-surface-2 rounded-xl gap-y-2 p-4">
            <DropDownOption optionName="Sort By" OptionIcon={SortByIcon} optionType="sortBy" optionValues={['name', 'amount']} options={budgetPlanStatsOptions} setOptions={setBudgetPlanStatsOptions}/>
            <DropDownOption optionName="Order By" OptionIcon={OrderByIcon} optionType="orderBy" optionValues={['asc', 'desc']} options={budgetPlanStatsOptions} setOptions={setBudgetPlanStatsOptions}/> 
        </div>
    )
}
...
function BudgetPlanStatsBreakDown(
    ...
    const sortedCategorizedExpenseListCards = sortedCategorizedExpenseListData.map((e, i) => {

        const [amountDollars, amountCents] = splitMoney(String(e.amount));

        return (
            <div key={e.category + i + ''} className="w-[95%] h-[4rem] flex flex-col justify-between items-center rounded-xl">
        ...
        <div className="w-full min-h-fit flex flex-col justify-center items-center bg-dark-surface-2 rounded-xl" >
            ...
            <div className="w-full flex flex-col items-center">
                ...
                <div className="w-full flex flex-col justify-center items-center">
                    <div className="w-full flex items-center justify-between h-[3.5rem] px-4" onClick={() => setBreakDownActive(prev => !prev)}>
                        <p className="text-lg font-bold">Breakdown</p>
                        {breakDownActive ? <ChevronUpIcon /> : <ChevronDownIcon />}
                    </div>
                    {
                        breakDownActive && (
...
export default function Main(
    ...
    const [budgetPlanStatsOptions, setBudgetPlanStatsOptions] = useState<sortOptionsType>(
        {
            sortBy: 'amount',
            orderBy: 'desc'
        }
    ...
    const sortedCategorizedExpenseListData = sortCategorizedList(categorizeExpenseList(expenseListData), budgetPlanStatsOptions.sortBy, budgetPlanStatsOptions.orderBy);
    ...
    return (
        <div className="w-full flex flex-col items-center justify-center gap-y-[1rem] py-[1rem] px-[1rem] bg-dark-surface-1 border-[1px] border-dark-border rounded-xl">
                <BudgetPlanStatsHeader budgetPlanStatsOptions={budgetPlanStatsOptions} setBudgetPlanStatsOptions={setBudgetPlanStatsOptions}/>
                <BudgetPlanStatsChart 
                    expenseListData={expenseListData}
                />
                <BudgetPlanStatsBreakDown
                    sortedCategorizedExpenseListData={sortedCategorizedExpenseListData} 
                />
        </div>
    )
}
